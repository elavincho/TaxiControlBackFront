...
import { SeguroRecord, AlertNotification } from '../src/types';
import { 
  getStoredSeguro, 
  getStoredAlertas, 
  getTodayDateString
...
export default function Seguro({ userId }: SeguroProps) {
  const [records, setRecords] = useState<SeguroRecord[]>(() => getStoredSeguro(userId));
  ...
  useEffect(() => {
    ...
    getSeguro(userId)
      ...
      .catch(err => console.error("Error loading Seguro:", err))
      .finally(() => {
        if (active) setLoading(false);
      });

    const handleStorageUpdate = () => {
      getSeguro(userId).then((data) => {
        if (active) setRecords(data);
      });
    };
  ...
  const [editingRecord, setEditingRecord] = useState<SeguroRecord | null>(null);
  ...
  const handleOpenEditModal = (rec: SeguroRecord) => {
    setEditingRecord(rec);
    setFechaPago(rec.fechaPago);
    setImporte(rec.importe.toString());
    setFechaVencimiento(rec.fechaVencimiento);
    setAseguradora(rec.aseguradora || '');
    setFormError('');
    setIsModalOpen(true);
  };

  const syncAlertForRecord = async (record: SeguroRecord, isDeleted = false, oldVencimiento?: string) => {
    try {
      ...
      if (!isDeleted) {
        await saveAlerta({
          userId: userId,
          tipo: 'seguro',
          mensaje: `Pago de póliza de seguro de Taxi mensual (${record.aseguradora || 'Aseguradora'}): $${record.importe}`,
          fechaLimite: record.fechaVencimiento,
          resuelta: false
        });
      }
    } catch (e) {
      console.error("Error syncing alert for seguro:", e);
    }
  };

  const handleDeleteRecord = async (id: string) => {
    if (window.confirm('¿Está seguro de que desea eliminar este registro de Seguro?')) {
      const recordToDelete = records.find(r => r.id === id);
      await deleteSeguro(id, userId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fechaPago || !importe || !fechaVencimiento || !aseguradora) {
    ...
    try {
      let savedRecord: SeguroRecord;
      if (editingRecord) {
        // Update
        const oldVencimiento = editingRecord.fechaVencimiento;
        savedRecord = await updateSeguro(editingRecord.id, { fechaPago, importe: numImporte, fechaVencimiento, aseguradora }, userId);
        setRecords(records.map(r => r.id === editingRecord.id ? savedRecord : r));
        await syncAlertForRecord(savedRecord, false, oldVencimiento);
      } else {
        // Create
        savedRecord = await saveSeguro({ userId, fechaPago, importe: numImporte, fechaVencimiento, aseguradora });
      ...
      setFormError(err.message || 'Error al guardar seguro.');
    }
  };
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      ...
      (r.aseguradora && r.aseguradora.toLowerCase().includes(q))
    );
  });
  ...
  return (
    <div className="space-y-6" id="seguro-section">
      {/* 1. Page Header & Stats Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight font-display">Seguro Automotor</h1>
          <p className="text-sm text-slate-500">Control de póliza de seguro, pagos mensuales y vencimientos de cobertura.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center space-x-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Historial de Pagos</p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center space-x-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Último Importe</p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center space-x-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Pólizas Pagadas</p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-100 flex flex-col sm:flex-row gap-3 items-center justify-between">
          <div className="relative w-full sm:max-w-xs">
            ...
            <input 
              type="text"
              placeholder="Buscar por importe, fecha..."
              value={searchQuery}
        ...
        {filteredRecords.length > 0 ? (
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50 text-slate-400 font-extrabold text-[10px] uppercase tracking-wider">
                  ...
                  <th className="py-4 px-6">Aseguradora</th>
                ...
                {filteredRecords.map((rec) => (
                  <tr key={rec.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3.5 px-6 font-semibold text-slate-700">
                      {rec.aseguradora || 'No especificada'}
                    </td>
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-slate-400 font-bold">No se encontraron registros de Seguro</p>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-100">
            <div className="flex items-center justify-between p-5 bg-slate-50 border-b border-slate-100">
              <h3 className="font-black text-slate-950 text-base font-display">
                {editingRecord ? 'Editar Pago de Seguro' : 'Registrar Pago de Seguro'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {/* Aseguradora */}
              <div className="space-y-1.5">
                <label className="text-xs font-black text-slate-400 uppercase tracking-wider block">Aseguradora / Compañía</label>
                <input 
                  type="text"
                  placeholder="Ej: Sancor Seguros, Rivadavia..."
                  value={aseguradora}
