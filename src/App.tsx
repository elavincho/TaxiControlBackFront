...
export default function App() {
  ...
  const [initialFuelType, setInitialFuelType] = useState<'GNC' | 'Nafta' | null>(null);
  const [initialMaintTrigger, setInitialMaintTrigger] = useState<boolean>(false);

  // Inactivity session expiration (5 minutes = 300 seconds)
  const [isSessionTimeoutWarning, setIsSessionTimeoutWarning] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    activityEvents.forEach(event => window.addEventListener(event, resetTimerOnUserAction));

    // First trigger to boot up
    resetTimerOnUserAction();

    return () => {
      activityEvents.forEach(event => window.removeEventListener(event, resetTimerOnUserAction));
      if (userActivityTimeoutRef.current) clearTimeout(userActivityTimeoutRef.current);
      if (timerCountdownRef.current) clearInterval(timerCountdownRef.current);
    };
  }, [currentUser]);
  ...
  const handleLogoutDueToInactivity = () => {
    ...
    alert('Tu sesión ha expirado automáticamente por inactividad para proteger tus datos financieros.');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row relative" id="applet-viewport">
        ...
        <div className="space-y-4 pt-4 border-t border-zinc-900">
          <div className="flex items-center space-x-3 px-2 bg-zinc-900/50 p-2 rounded-xl border border-zinc-800/40">
            ...
            <div className="truncate text-xs">
              <strong className="text-slate-200 block truncate">{currentUser.name}</strong>
              <span className="text-slate-500 text-[10px] font-mono block truncate">@{currentUser.username}</span>
            </div>
          </div>
          ...
          >
            {theme === 'dark' ? <Sun className="h-4 w-4 text-yellow-400 shrink-0" /> : <Moon className="h-4 w-4 text-slate-400 shrink-0" />}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 p-6 rounded-2xl w-full max-w-md text-center space-y-4 shadow-xl relative text-slate-900">
            ...
            <div className="text-4xl font-mono font-black text-yellow-600 animate-pulse">
              {inactivityTimer} <span className="text-sm font-semibold">segundos</span>
            </div>

            <p className="text-[10px] text-slate-400 italic">
              Mueve el mouse, presiona una tecla o toca la pantalla para cancelar y renovar tu sesión de forma segura.
            </p>
          </div>
        </div>
      )}
